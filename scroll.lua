...
function scroll:draw(wibox, cr, width, height)
    ...
    -- On dessine que le reste du widget (width-self._x > self._wg and self._wg or width-self._x ) sinon on dessin toute la place aloué width-self._x
...
local function new(widget, deltax, space, width, autoscroll, expanded)
    ...
    ret._scrolltimer = timer({ timeout = 1 })
